...
serve(async (req: Request) => {
  try {
    const { url, comprovanteId } = await req.json();
    ...
    // Step 2: Get appropriate parser and parse the data
    ...
    // Step 3: Update the comprovante in database
    ...
    // Update comprovante with error status
    try {
      const reqClone = req.clone();
      const { comprovanteId } = await reqClone.json().catch(() => ({ comprovanteId: null }));
      if (comprovanteId) {
        await supabase
          .from("comprovantes")
          .update({
